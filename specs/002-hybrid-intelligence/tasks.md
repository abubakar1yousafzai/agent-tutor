...
- [ ] T001 Add `openai-agents` to `backend/requirements.txt` (new line after `jsonschema>=4.0.0`)
- [ ] T002 [P] Add `GEMINI_API_KEY=your_gemini_api_key_here` block to `backend/.env.example` with a `# Phase 2 — Hybrid Intelligence` comment header
...
- [ ] T003 Create `backend/agents/__init__.py` as an empty file (module marker only — no content)
